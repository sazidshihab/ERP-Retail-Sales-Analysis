...
select  count(distinct stock_code) from stock_selling_group_;
select count(distinct stock_code) from stock_selling_group;


#making data for analyzing profit/loss table 
create or replace view stock_selling_group_ as 
select date_format(date1,'%Y-%m') as date1, stock_code, details, round(sum(average_cost),2) as average_cost, round(sum(total_sale),2) as total_sale , round(sum(quantity),2) as quantity, round(sum(tax),2) as tax, discontinue
...
select date1, stock_code, details,average_cost, total_sale, quantity, tax,  grp2, cost, cost2, (
case 
when average_cost is null then cost2
when cost2 is null then average_cost/quantity
when abs(total_sale/quantity - average_cost/quantity) < abs(total_sale/quantity - cost2) and total_sale/quantity > average_cost/quantity  and total_sale/quantity > cost2 then average_cost/quantity
when abs(total_sale/quantity - average_cost/quantity) > abs(total_sale/quantity - cost2) and total_sale/quantity > cost2 and total_sale/quantity > average_cost/quantity then cost2
when abs(total_sale/quantity - average_cost/quantity) < abs(total_sale/quantity - cost2) and total_sale/quantity < average_cost/quantity and total_sale/quantity < cost2 then average_cost/quantity
when abs(total_sale/quantity - average_cost/quantity) > abs(total_sale/quantity - cost2) and total_sale/quantity < average_cost/quantity and total_sale/quantity < cost2 then cost2
when average_cost/quantity > total_sale/quantity and cost2<total_sale/quantity then cost2
when average_cost/quantity < total_sale/quantity and cost2>total_sale/quantity then average_cost/quantity
...
select * from stock_selling_group_;
select count(distinct stock_code) from profit_loss_pre;
...
select count(distinct stock_code) from profit_loss;
